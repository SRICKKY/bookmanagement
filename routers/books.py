...
async def create_book(book: schemas.BookCreate, db: AsyncSession = Depends(get_session_local)):
...
async def update_book(id: int, book: schemas.BookUpdate, db: AsyncSession = Depends(get_session_local)):
...
async def delete_book(id: int, db: AsyncSession = Depends(get_session_local)):
...
async def create_review(id: int, review: schemas.ReviewCreate, db: AsyncSession = Depends(get_session_local)):
    return await crud.create_review(db=db, book_id=id, review=review)
